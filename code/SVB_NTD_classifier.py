...
import sys
#run as
print("usage : python *.py rank=5 batch_size=256 dataset=dblp")
...
class SGPTF_CLF:
    ...
    def __init__(self, init_config):
        ...
        with tf.variable_scope( self.PARAS_SCOPE_NAME):
            ...
            self.tf_mu_U = [tf.Variable(np.random.randn( num_elem, self.rank)*0.01, dtype=FLOAT_TYPE) for num_elem in self.elem_sizes]
        ...
        #self.log_lengthscale_old_ori = tf.Variable( np.zeros( shape = [self.rank_psd_input,1]), dtype=FLOAT_TYPE)
        #self.log_lengthscale_old = tf.stop_gradient(self.log_lengthscale_old_ori )
        #self.log_amp_old_ori = tf.Variable( 0, dtype=FLOAT_TYPE)
        #self.log_amp_old = tf.stop_gradient(self.log_amp_old_ori )
        ...
        '''batch_post_mu_U = common_funcs.get_embeddings(self.tf_mu_U, self.batch_inds)
        batch_post_std_vec_U = common_funcs.get_embeddings(self.tf_std_vec_U, self.batch_inds)
        batch_mu_U_old = common_funcs.get_embeddings( self.mu_U_old, self.batch_inds)
        batch_std_vec_U_old = common_funcs.get_embeddings(self.std_vec_U_old, self.batch_inds)
        '''
    ...
    def fit_batch(self, inds, rates, steps, print_every = 100, clean_hist = True, verbose = True ):
        ...
        for step in range( 1, steps + 1):
            ...
            ELBO, sampled_f, data_fidelity,KL_U, KL_B, KL_b, batch_U_mean, batch_U_std_vec, _  = self.sess.run( [
                self.ELBO, self.post_sample_f,  self.data_fidelity,self.KL_U, self.KL_B, self.KL_b, self.batch_mean, self.batch_std_vec, self.min_step], feed_dict= train_feed, options= self.run_options)
            ...
            if step % print_every == 0 and verbose:
                ...
                print('\nstep = %d, ELBO = %g, data_fidelity = %g, -KL_U = %g, -KL_b = %g, -KL_B = %g,train_auc = %g' % (
                step, ELBO, data_fidelity, -KL_U, -KL_b, -KL_B,train_auc))

        end_time = time.time()

        if verbose:

            print('secs_per_entry = %e\n' % (( end_time - start_time) / len( inds)))
...
def main():
    ...
    log_file = common_funcs.init_log_file('svb_classifier_by_%s.txt' % user_name, data_name, init_config)
    init_config['log_file'] = log_file
    model = SGPTF_CLF(init_config)
    ...
    for n_batch in range(1, num_batches + 1):
        ...
        if verbose:
            ...
            print("batch = %d, progress = %4.3g, test_acc = %g, test_auc = %g, ave_llk = %g\n" % ( n_batch, n_batch / num_batches * 100, acc, auc, ave_test_llk))
    ...
    print("\nbatch = %d, progress = %4.3g, test_acc = %g, test_auc = %g, ave_llk = %g" % ( n_batch, n_batch / num_batches * 100, acc, auc, ave_test_llk))
    log_file.write("batch = %d, progress = %4.3g, test_acc = %g, test_auc = %g, ave_llk = %g\n" % ( n_batch, n_batch / num_batches * 100, acc, auc, ave_test_llk))
    log_file.close()
